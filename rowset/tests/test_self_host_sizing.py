...
_WRITE_BENCHMARK = _REPO_ROOT / "deployment" / "self-host" / "write-benchmark.py"
...
def test_benchmark_writer_emits_the_capacity_named_schema(tmp_path):
    output = tmp_path / "benchmark.json"
    subprocess.run(
        [
            sys.executable,
            str(_WRITE_BENCHMARK),
            str(output),
            "--platform",
            "linux/amd64",
            "--os-id",
            ...
            "test-run",
            "--image-reference",
            "ghcr.io/lvtd-llc/rowset:0123456789012345678901234567890123456789",
            "--image-digest",
            f"sha256:{'0' * 64}",
            "--image-indexes",
            "[]",
    ...
    assert benchmark["schema_version"] == 2
    assert benchmark["host"]["disk_capacity_bytes"] == 39_964_635_136
    assert "disk_bytes" not in benchmark["host"]
