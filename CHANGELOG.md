...
### Changed
- Production deploy now builds one server/worker image in GitHub Actions, publishes it to GHCR with `latest`, UTC date, date-run, and short-SHA tags, then deploys that image to both CapRover apps with per-app deploy tokens.
