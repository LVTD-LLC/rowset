...
ROWSET_AGENT_SETUP_INSTRUCTIONS = (
    ...
    "not print it, commit it, or paste it back to chat. Configure Rowset as a "
    "remote Streamable HTTP MCP server named rowset, using the Rowset MCP URL "
    "above and bearer-token env var ROWSET_API_KEY so requests send "
    "Authorization: Bearer <key>. For Codex/OpenClaw-compatible clients, the "
    "setup command is: codex mcp add rowset --url <Rowset MCP URL> "
    "--bearer-token-env-var ROWSET_API_KEY. If the client only supports custom "
    "headers, set Authorization to Bearer <key>; use X-API-Key only for REST "
    "clients that cannot send bearer tokens. After setup, call get_user_info to "
    "verify authentication, then call get_all_datasets to discover available "
    "datasets. If auth fails, confirm the env var contains the full key, not "
    "only its prefix. Use create_dataset when you need to create a dataset on "
    "the fly. Use update_dataset_public_preview when the user asks for a "
    "shareable read-only preview. Discover the current MCP tools and API docs "
    "at runtime before working with datasets."
)
