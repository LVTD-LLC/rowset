...
from apps.datasets.model_typing import (
    dataset_asset_objects,
    dataset_mutation_objects,
    dataset_row_mutation_dataset_fields,
    dataset_row_mutation_fields,
    dataset_row_objects,
)
...
def create_dataset_row(
    ...
    *,
    ...
    serialized_data = normalize_row_data_for_headers(row_data, dataset_fields.headers)
    ...
    row = dataset_row_objects().create(
        dataset=dataset,
    ...
    dataset_fields.row_count = dataset_fields.rows.count()
    dataset_fields.updated_by_agent_api_key = agent_api_key
    ...
    record_dataset_mutation(
        dataset,
        DatasetMutationType.ROW_CREATED,
        f"Row {row_fields.row_number} added.",
        agent_api_key=agent_api_key,
        target_type="row",
        target_identifier=row_fields.id,
        metadata={
            "row_id": row_fields.id,
            "row_number": row_fields.row_number,
            "changed_fields": sorted(row_fields.data or {}),
        },
    )
    track_dataset_row_mutation(
        ...
        changed_field_count=len(row_fields.data or {}),
        track_activation_event_func=hooks.track_activation_event,
    )
    hooks.enqueue_dataset_row_vector_index(row_fields.id)
    row = dataset_fields.rows.prefetch_related("assets").get(id=row_fields.id)
    return {
        "status": "success",
        "message": "Row created.",
...
def patch_dataset_row(
    ...
    *,
    ...
    dataset_fields = dataset_row_mutation_dataset_fields(dataset)
    row_fields = dataset_row_mutation_fields(row)
    row_patch = normalize_row_patch_for_headers(data, dataset_fields.headers)
    ...
    current_data = row_fields.data or {}
    ...
    row_fields.data = {**current_data, **row_patch}
    index_changed = _apply_index_patch(dataset, row, row_patch, data, hooks)
    hooks.validate_relationship_row_data(dataset, row_fields.data, columns=patched_fields)
    _delete_cleared_image_assets(dataset, row, cleared_image_columns)
    row_fields.updated_by_agent_api_key = agent_api_key
    ...
    record_dataset_mutation(
        dataset,
        DatasetMutationType.ROW_UPDATED,
        f"Row {row_fields.row_number} updated.",
        agent_api_key=agent_api_key,
        target_type="row",
        target_identifier=row_fields.id,
        metadata={
...
def delete_dataset_row(
    ...
    *,
    ...
    hooks.enqueue_dataset_row_vector_delete(dataset_fields.id, [row_id])
...
def delete_dataset_rows(
    ...
    *,
    ...
    for row in ordered_rows:
        row_fields = dataset_row_mutation_fields(row)
        deleted_rows.append((row_fields.id, row_fields.row_number))
    dataset_fields.rows.filter(id__in=ordered_row_ids).delete()
    dataset_fields.row_count = dataset_fields.rows.count()
    dataset_fields.updated_by_agent_api_key = agent_api_key
    dataset.save(update_fields=["row_count", "updated_by_agent_api_key", "updated_at"])
    is_first_row_mutation = not profile_has_row_mutation(profile)
    ...
    hooks.enqueue_dataset_row_vector_delete(
        dataset_fields.id,
        [row_id for row_id, _ in deleted_rows],
    )
...
def _next_row_number(dataset: Dataset) -> int:
    dataset_fields = dataset_row_mutation_dataset_fields(dataset)
    last_row_number = (
        dataset_fields.rows.order_by("-row_number").values_list("row_number", flat=True).first()
        or 0
    )
...
def _create_row_data(dataset: Dataset, data: RowWritePayload, row_number: int) -> RowWritePayload:
    dataset_fields = dataset_row_mutation_dataset_fields(dataset)
    if dataset_fields.index_generated:
        return {**data, dataset_fields.index_column: str(row_number)}
    return data


def _required_create_index_value(dataset: Dataset, serialized_data: RowData) -> str:
    dataset_fields = dataset_row_mutation_dataset_fields(dataset)
    index_value = str(serialized_data.get(dataset_fields.index_column, "")).strip()
    if not index_value:
        raise DatasetServiceError(
            400,
            f"Index column '{dataset_fields.index_column}' is required.",
        )
    return index_value
...
def _raise_if_duplicate_index(dataset: Dataset, index_value: str) -> None:
    dataset_fields = dataset_row_mutation_dataset_fields(dataset)
    if dataset_fields.rows.filter(index_value=index_value).exists():
...
def _changed_image_columns(
    ...
    row_fields = dataset_row_mutation_fields(row)
    image_columns = set(
        image_columns_from_schema(dataset_fields.headers, dataset_fields.column_schema)
    )
    return [
        field
        for field in patched_fields
        if field in image_columns
        and row_patch.get(field, "") != str((row_fields.data or {}).get(field, "") or "")
    ]
...
def _apply_index_patch(
    ...
    if not index_value:
        raise DatasetServiceError(
            400,
            f"Index column '{dataset_fields.index_column}' cannot be blank.",
        )
...
def _ordered_rows_for_delete(dataset: Dataset, ordered_row_ids: list[int]) -> list[DatasetRow]:
    dataset_fields = dataset_row_mutation_dataset_fields(dataset)
    rows_by_id = {
        dataset_row_mutation_fields(row).id: row
        for row in dataset_fields.rows.filter(id__in=ordered_row_ids)
    }
    ordered_rows = []
    ...
    *,
    ...
    track_activation_event_func(
        ...
        ROWSET_DATASET_ROW_MUTATED,
        {
            ...
            "row_count_after": dataset_fields.row_count,
