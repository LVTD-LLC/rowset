# Rowset field notes

Practical writing about AI agent infrastructure, MCP databases, dataset APIs, and workflows that
keep structured data useful.

## Guides

- [What is an agent-managed dataset?]({{ site_url }}/blog/agent-managed-datasets.md)
- [How to connect an AI agent to the Rowset Dataset API]({{ site_url }}/blog/connect-ai-agent-to-dataset-api.md)
- [When should an AI agent use MCP instead of REST?]({{ site_url }}/blog/mcp-vs-rest-ai-agents.md)
- [How to choose an index column for agent-managed rows]({{ site_url }}/blog/choose-index-column-agent-rows.md)
- [Rowset `rowset_id` vs business keys]({{ site_url }}/blog/rowset-id-vs-business-keys.md)
- [How to structure dataset instructions for AI agents]({{ site_url }}/blog/structure-dataset-instructions-ai-agents.md)
- [How to model relationships between agent-managed datasets]({{ site_url }}/blog/relationship-modeling-agent-datasets.md)
- [Human-in-the-loop AI agents: a practical workflow]({{ site_url }}/blog/human-in-the-loop-ai-agents.md)
- [AI agent audit trail: what to log and how to build it]({{ site_url }}/blog/ai-agent-audit-trail.md)
- [How to choose a database for AI agents]({{ site_url }}/blog/database-for-ai-agents.md)
- [Best Airtable alternatives for AI-agent-managed datasets]({{ site_url }}/blog/airtable-alternatives.md)
- [Best Baserow alternatives for AI-agent-managed datasets]({{ site_url }}/blog/baserow-alternatives.md)
- [Best Google Sheets alternatives for AI-agent-managed datasets]({{ site_url }}/blog/google-sheets-alternatives.md)
- [Best NocoDB alternatives for AI-agent-managed datasets]({{ site_url }}/blog/nocodb-alternatives.md)
