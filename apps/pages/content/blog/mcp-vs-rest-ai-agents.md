...
access](/how-to/connect-mcp/). If MCP is unavailable, use the [Dataset
API](/docs/dataset-api/) with `Authorization: Bearer <key>`.
...
rate-limited APIs
([MCP authorization guide](https://modelcontextprotocol.io/tutorials/security/authorization)).
...
Start with [agent access](/how-to/configure-agent-access/) if you are configuring a
trusted assistant. Use [MCP access](/how-to/connect-mcp/) when the client supports
it. Keep the [Dataset API](/docs/dataset-api/) close for scripts,
