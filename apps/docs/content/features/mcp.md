...
source spreadsheet when the user has connected Google with Sheets access. Deployments
can also fall back to `GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON` when the sheet is shared
with that service account as an editor. Public Google Sheets CSV import by itself is
read-only.
