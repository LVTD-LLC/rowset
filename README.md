...
For a self-hosted installation, Qdrant is bundled and its key is generated by
`deployment/self-host/init-env.sh`. Set the vector-search flag and a private `OPENROUTER_API_KEY`,
then start normally. Backfill every active dataset after enabling it:
...
When `ROWSET_VECTOR_SEARCH_ENABLED=True`, the supported lifecycle also starts the pinned, private,
API-key-protected `qdrant` service through the `vector-search` Compose profile.
