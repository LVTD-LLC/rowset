...
- Update `CHANGELOG.md` for notable user-facing features, behavior changes, bug
  fixes, security changes, meaningful operational changes, and substantive
  documentation improvements. Do not add entries for SEO work, routine content
  publishing, blog posts, tutorials, marketing pages, content briefs, keyword
  research, or internal-link changes. Do not add entries for tests, refactors,
  formatting, or internal maintenance unless they materially affect users or
  operators. Add entries under the current calendar date using the existing
