...
```bash
uv run ty check apps/core/capabilities.py apps/core/agent_skill.py rowset/utils.py rowset/logging_utils.py
```

These modules are low-noise because they are mostly pure helpers or structured
agent-facing output code. They should stay clean in CI.
