...
- Each visible tag renders as a pill with a deterministic color derived from
...
item contains the trimmed label and one deterministic class from the existing
choice-pill color palette. This display metadata is separate from the cell's
original value. A shared template partial renders the list so all four table
and row-detail surfaces use the same markup.
...
5. Server-rendered dataset views derive nonblank trimmed labels for display and
   render deterministic colored pills.
